...
void Print_vector10(double b[], int n, char title[]);
void Vector_sum(double x[], double y[], double z[], int n);

void Generate_random_vector(double a[], int n)
{
    for (int i = 0; i < n; i++)
        a[i] = (double)rand() / RAND_MAX; // Si deseas números entre 0 y 999, puedes simplemente usar a[i] = rand();
}
/*---------------------------------------------------------------------*/
int main(void)
{
    ...
    // for (int i = 0; i < n; i++)
    // {
    //     x[i] = rand(); // Números aleatorios entre 0 y 999
    //     y[i] = rand();
    // }
    Generate_random_vector(x, n);
    Print_vector10(x, n, "Head and tail of the first vector are");
    Generate_random_vector(y, n);
    Print_vector10(y, n, "Head and tail of the second vector are");

    Vector_sum(x, y, z, n);

    // Print_vector(z, n, "The sum is");
    Print_vector10(z, n, "Head and tail of the sum are");
    end = clock();
    cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
    printf("Tiempo utilizado: %f sec\n", cpu_time_used);
    free(x);
    free(y);
    free(z);
...
void Print_vector10(double b[], int n, char title[])
{
    int i;
    printf("%s\n", title);
    for (i = 0; i < 10; i++) // Imprimir primeros 10 elementos
        printf("%f ", b[i]);
    printf("\n...\n");           // Indica que hay más elementos que no se están mostrando
    for (i = n - 10; i < n; i++) // Imprimir últimos 10 elementos
        printf("%f ", b[i]);
    printf("\n");
} /*---------------------------------------------------------------------
   * Function:  Vector_sum
