...
void Print_vector(double local_b[], int local_n, int n, char title[],
                  int my_rank, MPI_Comm comm);
void Print_vector10(double local_b[], int local_n, int n, char title[],
                    int my_rank, MPI_Comm comm);
...
int main(void)
{
    ...
    Print_vector10(local_y, local_n, n, "y is", my_rank, comm);

    Parallel_vector_sum(local_x, local_y, local_z, local_n);
    tend = MPI_Wtime();
    Print_vector10(local_z, local_n, n, "The sum is", my_rank, comm);
    double dot_product = Parallel_dot_product(local_x, local_y, local_n, comm);
    if (my_rank == 0)
    {
...
double Parallel_dot_product(double local_x[], double local_y[], int local_n, MPI_Comm comm)
{
    ...
    // Calcular producto punto local
    for (int i = 0; i < local_n; i++)
    {
        local_dot_product += local_x[i] * local_y[i];
    }

    // Reducir los productos locales en un producto global
